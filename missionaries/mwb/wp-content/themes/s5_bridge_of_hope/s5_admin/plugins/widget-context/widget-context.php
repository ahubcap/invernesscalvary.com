...
new s5_widget_context();



class s5_widget_context {



	var $options_name = 's5_wid_ctxt'; // Widget context settings (visibility, etc)
	...
	function s5_widget_context() {

		$this->plugin_path = get_bloginfo('template_directory').'/s5_admin/plugins/widget-context/';



		// Amend widget controls with Widget Context controls

		add_action('sidebar_admin_setup', array($this, 'attach_s5_widget_context_controls'));
	...
	function save_s5_widget_context() {
		...
		$new_s5_widget_context_settings = array_values($_POST['wl']);



		// Add/Update

		foreach($new_s5_widget_context_settings as $widget_id => $s5_widget_context) {

			if (empty($s5_widget_context))

				$s5_widget_context = array();

			// If neither type of widget logic behaviour is selected, set to default

			if (!isset($s5_widget_context['incexc']))

				$s5_widget_context['incexc'] = 'notselected';



			$options[(string)$_POST['widget-id']] = $s5_widget_context;
	...
	function attach_s5_widget_context_controls() {
	...
	function replace_widget_control_callback() {
		...
		print $this->display_s5_widget_context($original_callback, $widget_id);
	...
	function display_s5_widget_context($args = array(), $wid = null) {
