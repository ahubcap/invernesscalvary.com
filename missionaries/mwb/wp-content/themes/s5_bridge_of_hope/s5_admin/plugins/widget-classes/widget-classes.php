...
add_action('in_widget_form',         array('s5_widget_styles', 'in_widget_form'), 10, 3);
add_filter('widget_update_callback', array('s5_widget_styles', 'widget_update_callback'), 10, 4);
//add_filter('dynamic_sidebar_params', array('s5_widget_styles', 'dynamic_sidebar_params'));
add_filter('sidebar_admin_setup',    array('s5_widget_styles', 'sidebar_admin_setup'));

class s5_widget_styles {
	/**
	...
	function dynamic_sidebar_params($params) {
		...
		if (! is_array($widget['callback'])) {
			$options = get_option('s5_widget_styles', array());
	...
	function intercept($widget_id) {
		...
		if ($old_class != $new_class) {
			update_option('s5_widget_styles', $options);
		}
	...
	function in_widget_form($widget, $return, $instance) {
		...
		$wid_id = str_replace('-class','',str_replace('widget-','',$widget->get_field_id('class')));
		$return = null;
		self::form($widget->get_field_id('class'), $widget->get_field_name('class'), $class, $wid_id);
	}
...
<?php
	...
	function widget_update_callback($instance, $new_instance, $old_instance, $widget) {
		global $s5_wid_style_id;
		if (array_key_exists('class', $new_instance)) {
			$instance['class'] = str_replace(',', ' ', $new_instance['class']);
		}
			$option = get_settings('s5_widget_styles');
			$wid_id = str_replace('-class','',str_replace('widget-','',$widget->get_field_id('class')));
			$option[$wid_id] = $instance['class'];
			update_option('s5_widget_styles', $option);
